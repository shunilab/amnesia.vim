...
見出し・箇条書き・チェックボックス・引用・コードブロック・テーブル等、行単位で意味を持つコマンドは、ビジュアル選択が行の一部（文字単位）であっても選択範囲の行全体に適用される。`MDBold` / `MDItalic` / `MDInlineCode` / `MDStrike` / `MDHighlight` / `MDLink` / `MDImage` / `MDFootnote` はブロック選択（Ctrl-V）に対応しておらずエラーになる。

| コマンド | 動作 |
|---|---|
| `:MDH1` 〜 `:MDH6` | 見出しレベルを設定（同レベル再実行でトグル）。明示的な Ex 範囲指定は非対応 |
...
| `:MDBold` / `:MDItalic` / `:MDInlineCode` / `:MDStrike` / `:MDHighlight` | `**` / `*` / `` ` `` / `~~` / `==` でラップ（再実行でトグル）。明示的な Ex 範囲指定は非対応 |
